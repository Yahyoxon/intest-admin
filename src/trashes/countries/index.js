...
import { Table, Pagination, Modal, Header, PageContainer } from "components";
...
const List = ({ history, location }) => {
	...
	return (
		<>
			...
			/>
			<Modal.Default header="Добавить страны" toggle={createModal} setToggle={setCreateModal}>
				{createModal && <Create {...{ setCreateModal }} />}
			</Modal.Default>
			<Modal.Default header="Измените страны" toggle={updateModal} setToggle={setUpdateModal}>
			...
			<EntityContainer.All
				...
				{({ items, isFetched, meta }) => {
					return (
						<PageContainer isLoading={!isFetched} pageTitle={<>{t("Список стран")}</>}>
							<Header hasFilter={false} hasButton={false} btnName="Добавить" btnClick={() => setCreateModal(true)} />
							<Table
								items={items}
								rowKey="id"
								className="lg:mt-5"
