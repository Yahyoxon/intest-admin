...
const List = ({ history, location }) => {
	...
	return (
		<>
			...
			/>
			<Modal.Default header="Добавить тип дороги" toggle={createModal} setToggle={setCreateModal}>
				<Create {...{ setCreateModal }} />
			</Modal.Default>
			<Modal.Default header="Изменить тип дороги" toggle={updateModal} setToggle={setUpdateModal}>
				...
				{({ items, isFetched, meta }) => {
					return (
						<div className="pageContainer">
							<Header btnName="Добавить" title="Типы дорог" btnClick={() => setCreateModal(true)} meta={meta} hasFilter={false} />
							<Table
								items={items}
								rowKey="id"
								className="mt-5"
								...
								columns={[
									...
									},
									{
										title: t("Название(uz)"),
										dataIndex: "name_uz",
										render: value => <>{truncate(value, { length: 60, separator: "" })}</>
									},
									{
										title: t("Название(ru)"),
										dataIndex: "name_ru",
